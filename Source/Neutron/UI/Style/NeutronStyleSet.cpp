...
#include "NeutronStyleSet.h"
#include "Neutron/Neutron.h"

#include "Materials/MaterialInterface.h"
#include "Materials/MaterialInstanceDynamic.h"

#define NEUTRON_STYLE_INSTANCE_NAME "NeutronStyle"
#define NEUTRON_STYLE_PATH "/Game/UI"

/*----------------------------------------------------
    Singleton
----------------------------------------------------*/

TSharedPtr<FSlateStyleSet> FNeutronStyleSet::Instance = NULL;

void FNeutronStyleSet::Initialize()
{
	FSlateStyleRegistry::UnRegisterSlateStyle(NEUTRON_STYLE_INSTANCE_NAME);
...
void FNeutronStyleSet::Shutdown()
{
...
TPair<FText, const FSlateBrush*> FNeutronStyleSet::GetKeyDisplay(const FKey& Key)
{
	if (Key.IsValid())
	{
		...
		// Return details
		const FSlateBrush* DisplayBrush = FNeutronStyleSet::GetBrush(*(FString(TEXT("Key/SB_")) + DisplayBrushName));
		NCHECK(DisplayBrush);
...
TPair<TSharedPtr<FSlateBrush>, UMaterialInstanceDynamic*> FNeutronStyleSet::GetDynamicBrush(const FString& BrushName)
{
	TSharedPtr<FSlateBrush> ResultBrush = MakeShared<FSlateBrush>();
	const FSlateBrush*      BaseBrush   = FNeutronStyleSet::GetBrush(BrushName);
...
FLinearColor FNeutronStyleSet::GetPlasmaColor(float Alpha)
{
...
FLinearColor FNeutronStyleSet::GetViridisColor(float Alpha)
{
...
TSharedRef<FSlateStyleSet> FNeutronStyleSet::Create()
{
	TSharedRef<FSlateStyleSet> StyleRef =
		FSlateGameResources::New(FName(NEUTRON_STYLE_INSTANCE_NAME), NEUTRON_STYLE_PATH, NEUTRON_STYLE_PATH);
	FSlateStyleSet& Style = StyleRef.Get();

	Style.Set("Neutron.Button", FButtonStyle()
