...
FNeutronMeshInterfaceBehavior::FNeutronMeshInterfaceBehavior()
...
void FNeutronMeshInterfaceBehavior::SetupMaterial(UPrimitiveComponent* Mesh, UMaterialInterface* Material)
{
	ComponentMaterial = UMaterialInstanceDynamic::Create(Material, Mesh);
	Mesh->SetMaterial(0, ComponentMaterial);
}

void FNeutronMeshInterfaceBehavior::SetupMaterial(UDecalComponent* Decal, UMaterialInterface* Material)
{
	ComponentMaterial = UMaterialInstanceDynamic::Create(Material, Decal);
}

void FNeutronMeshInterfaceBehavior::TickMaterial(float DeltaTime)
{
	...
	{
		float Alpha =
			FMath::InterpEaseInOut(0.0f, 1.0f, CurrentMaterializationTime / MaterializationDuration, ENeutronUIConstants::EaseStandard);
		ComponentMaterial->SetScalarParameterValue("MaterializeAlpha", Alpha);
	}

	// Process requests
	TArray<FNeutronMaterialParameterRequest> OngoingRequests;
	for (FNeutronMaterialParameterRequest& Request : CurrentRequests)
	{
		...
		if (Request.IsColor)
		{
			FLinearColor Value = FMath::InterpEaseInOut(PreviousParameterColorValues[Request.Name], Request.ColorValue,
				Request.Time / ParameterFadeDuration, ENeutronUIConstants::EaseStandard);
...
void FNeutronMeshInterfaceBehavior::Materialize(bool Force)
{
...
void FNeutronMeshInterfaceBehavior::Dematerialize(bool Force)
{
...
float FNeutronMeshInterfaceBehavior::GetMaterializationAlpha() const
{
	return CurrentMaterializationTime / MaterializationDuration;
}

void FNeutronMeshInterfaceBehavior::RequestParameter(FName Name, float Value, bool Immediate)
{
	if (Immediate)
	{
		ComponentMaterial->SetScalarParameterValue(Name, Value);

		// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : immediate value %.2f for '%s'", Value, *Name.ToString());
	}
	else
	{
		FNeutronMaterialParameterRequest Request(Name, Value);
		...
		{
			// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : ignoring value %.2f for '%s'", Value, *Name.ToString());
			return;
		}
		...
		// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : new value %.2f for '%s' (previously %.2f)", Value, *Name.ToString(),
		//	PreviousValue);
...
void FNeutronMeshInterfaceBehavior::RequestParameter(FName Name, FLinearColor Value, bool Immediate)
{
	if (Immediate)
	{
		ComponentMaterial->SetVectorParameterValue(Name, Value);

		// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : immediate value %.2f, %.2f, %.2f, %.2f for '%s'", Value.R, Value.G,
		// Value.B,
	...
	else
	{
		...
		if (RequestIndex != INDEX_NONE)
		{
			// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : ignoring value %.2f, %.2f, %.2f, %.2f for '%s'", Value.R, Value.G,
			// Value.B,
			//	Value.A, *Name.ToString());
		...
		// NLOG("FNeutronMeshInterfaceBehavior::RequestParameter : new value %.2f, %.2f, %.2f, %.2f for '%s' (previously %.2f, %.2f, %.2f,
		// %.2f)",
...
FVector INeutronMeshInterface::GetExtent() const
{
...
bool INeutronMeshInterface::MoveComponentHierarchy(UPrimitiveComponent* RootComponent, const FVector& OriginalLocation,
	const FVector& Delta, const FQuat& NewRotationQuat, bool bSweep, FHitResult* OutHit, ETeleportType Teleport,
