...
#include "NeutronMeshInterface.generated.h"
...
struct FNeutronMaterialParameterRequest
{
	FNeutronMaterialParameterRequest(FName N, float V) : Name(N), Time(0), IsColor(false), FloatValue(V), ColorValue()
	{}

	FNeutronMaterialParameterRequest(FName N, FLinearColor V) : Name(N), Time(0), IsColor(true), FloatValue(), ColorValue(V)
	{}

	bool operator==(const FNeutronMaterialParameterRequest& Other) const
	{
...
USTRUCT()
struct NEUTRON_API FNeutronMeshInterfaceBehavior
{
	GENERATED_BODY()

public:

	FNeutronMeshInterfaceBehavior();
...
protected:
	...
	TArray<FNeutronMaterialParameterRequest> CurrentRequests;
...
class UNeutronMeshInterface : public UInterface
{
	GENERATED_BODY()
};

/** Base mesh interface for static and skeletal components */
class NEUTRON_API INeutronMeshInterface
{
	GENERATED_BODY()
