...
#include "NeutronTurntablePawn.h"
...
#define LOCTEXT_NAMESPACE "ANeutronTurntablePawn"

/*----------------------------------------------------
    Constructor
----------------------------------------------------*/

ANeutronTurntablePawn::ANeutronTurntablePawn()
	: Super()
	, CurrentPanTarget(0)
	, CurrentTiltTarget(0)
	, CurrentPanAngle(0)
...
void ANeutronTurntablePawn::BeginPlay()
{
	NLOG("ANeutronTurntableActor::BeginPlay");
...
void ANeutronTurntablePawn::Tick(float DeltaTime)
{
	...
	float InterpolatedOffset   = FMath::InterpEaseInOut(PreviousOffset, CurrentOffset, Alpha, ENeutronUIConstants::EaseStandard);
	float InterpolatedDistance = FMath::InterpEaseInOut(PreviousDistance, CurrentDistance, Alpha, ENeutronUIConstants::EaseStandard);
...
void ANeutronTurntablePawn::ResetView()
{
	CurrentTiltAngle      = -45;
	CurrentPanAngle       = 135;
...
TPair<FVector, FVector> ANeutronTurntablePawn::GetTurntableBounds() const
{
	return TPair<FVector, FVector>(FVector::ZeroVector, FVector::ZeroVector);
}

void ANeutronTurntablePawn::PanInput(float Val)
{
	CurrentPanTarget += Val;
}

void ANeutronTurntablePawn::TiltInput(float Val)
{
	CurrentTiltTarget += Val;
}

void ANeutronTurntablePawn::ZoomIn()
{
...
void ANeutronTurntablePawn::ZoomOut()
{
	CurrentDistanceFactor += DistanceFactorIncrement;
	float MinDistanceFactor = DefaultDistanceFactor - NumDistanceFactorIncrements * DistanceFactorIncrement;
	CurrentDistanceFactor   = FMath::Clamp(CurrentDistanceFactor, MinDistanceFactor, DefaultDistanceFactor);
...
void ANeutronTurntablePawn::ResetZoom()
{
	CurrentDistanceFactor = DefaultDistanceFactor;
}
...
void ANeutronTurntablePawn::ProcessCamera(float DeltaTime)
{
	// Apply filter
	bool IsGamepad = UNeutronMenuManager::Get()->IsUsingGamepad();
